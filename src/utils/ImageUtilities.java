...
public class ImageUtilities {   
	...
	public static BufferedImage highlight(BufferedImage image, int which) {
		try {
			...
			Color color = new Color(ImageOps.values()[which].Red(), ImageOps.values()[which].Green(), ImageOps.values()[which].Blue(), 255 * GridConstants.TRANSPARENCY / 100);
			g.setColor(color);
			g.fillRect(ImageOps.values()[which].XCor(), ImageOps.values()[which].YCor(), image.getWidth(), image.getHeight());
		} catch (Exception e) {
