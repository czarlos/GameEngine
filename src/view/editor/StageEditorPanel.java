...
public class StageEditorPanel extends JTabbedPane {
	...
	private JScrollPane makeTab(String type) {
		...
		for (int n = 0; n < tileNames.size(); n++) {
			GameObjectPanel gop = new GameObjectPanel(type,
	...
	public void changeSelected(GameObjectPanel selected) {
		...
		myWorldManager.setActiveObject(
				myID - 1,
				selected.getType(),
				myWorldManager.get(selected.getType()).indexOf(
						selected.getName()));
	}
	...
	class EditListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			myTableDialog = new TableDialog(gtm, new DialogListener(myWM, gtm,
					myPanel, myType), myWM.getDialogList(myType));
